...
function max_days() {
  ...
  cuttoff_date_YMD="$(gdate --utc -d "$max_days days ago" +%Y%m%d)" # YYYYMMDD
  for ((i=0;i<${#REMOTE_VERSIONS[@]};i++)); do
...
function archive() {
  ...
  for ((i=0;i<${#REMOTE_VERSIONS[@]};i++)); do
    ...
    [ "$version_date_YMD" -gt "$(gdate --utc -d "$algorithm_last_changed" +%Y%m%d)" ] || continue

    if [ "$version_date_YMD" -ge "$(gdate --utc -d "7 days ago" +%Y%m%d)" ]; then
      # Keep every version from last 7 days
      print_action "7D: Keeping" "${REMOTE_VERSIONS[$i]}" "$created"
    elif [ "$version_date_YMD" -ge "$(gdate --utc -d "1 month ago" +%Y%m%d)" ]; then
    ...
    elif [ "$version_date_YMD" -ge "$(gdate --utc -d "3 months ago" +%Y%m%d)" ]; then
      ...
      if [ "${version_date_YMD:0:6}" != "${previous_version_date_YMD:0:6}" ]; then
        ...
        if [ "$version_date_YMD" -ge "$(gdate --utc -d "1 year ago" +%Y%m%d)" ]; then
...
's|.*"created":"([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z)".*|\1|p' )"
  ...
  version_date_YMD="$(gdate --utc -d "$created" +%Y%m%d)" # YYYYMMDD
  version_date_Ywk="$(gdate --utc -d "$created" +%Y%V)" # YYYY<weeknum>
  [ "$version_date_YMD" ] || return 1 # "created" field was wrong format
}

function print_action() {
  if [ "$3" ]; then
    echo "$1 version $2 ($(gdate --utc -d "$3" "+%c"))"
  else
    echo "$1 version $2"
  fi
}
