...
TEST_CASE("MachinetalkService Tests", "[machinetalk]")
{
    ...
    SECTION("creating motion object works") {
        QObject *output = MachinetalkService::recurseDescriptor(EmcStatusMotion::descriptor(), &engine, &parent);
        ...
        SECTION("updating bool value works") {
            ...
            MachinetalkService::recurseMessage(message, &engine, output);
        ...
        SECTION("position message also has a list of values") {
            const auto &value = output->property("actualPosition");
            QObject *object = qvariant_cast<QObject *>(value);

            REQUIRE(object != nullptr);
            const auto &data = qvariant_cast<QVariant>(object->property("data"));
            REQUIRE(data.toList().count() == 9);
        }
        ...
        SECTION("updating position list of values works") {
            ...
            const auto &value = output->property("actualPosition");
            QObject *object = qvariant_cast<QObject *>(value);
            const auto &data = qvariant_cast<QVariant>(object->property("data")).toList();
            REQUIRE(data.at(1).toDouble() == Approx(2.5));
        }
    ...
    SECTION("creating config object works") {
        QObject *output = MachinetalkService::recurseDescriptor(EmcStatusConfig::descriptor(), &engine, &parent);
    ...
    SECTION("creating launcher object works") {
        QObject *output = MachinetalkService::recurseDescriptor(Launcher::descriptor(), &engine, &parent);
        ...
        SECTION("updating File object creates temporary file") {
            ...
            MachinetalkService::recurseMessage(message, &engine, output, tempDir.path());
